...
app = typer.Typer(
    ...
    help="cloak — privacy-first PII scrubber",
)
...
def scan(
    ...
    cfg: CloakConfig = ctx.obj["config"]
...
def scrub(
    ...
    mode: Mode = typer.Option(
        Mode.pseudonymize, "--mode", help="Action to apply to entities", case_sensitive=False
    ),
):
    """Write a sanitized mirror of the source to OUT."""
    cfg: CloakConfig = ctx.obj["config"]
    pipeline = Pipeline(cfg, mode=mode.value)
...
def review(
    ctx: typer.Context,
    review_file: pathlib.Path = typer.Argument(..., help="review.jsonl to triage"),
):
...
@app.command()
def hook(ctx: typer.Context, cmd: str = typer.Argument(..., help="install|uninstall")):
